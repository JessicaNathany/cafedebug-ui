...
import { listUsersFromBackend } from "@/lib/api/users-admin-api";
...
export async function usersListHandler(request: Request) {
  ...
  addSentryBreadcrumb("Admin users list request", {
    category: "users",
    data: {
      module: "users",
      action: "list",
  ...
  try {
    const backendResult = await listUsersFromBackend({
      cookieHeader,
    ...
    captureException(error, {
      scope: { tags: { module: "users", action: "list" }, level: "error" },
    ...
    return createUsersErrorResponse({
      ...
      detail: "Unable to load users right now.",
      setCookieHeaders: []
    });
  }
}
