---
description: This skill enables visual inspection of websites running locally or remotely to identify and fix design issues. In Cafe Debug, it must use `cafedebug.pen` as the visual source of truth via Pencil MCP. Triggers on requests like "review website design", "check the UI", "fix the layout", "find design problems". Detects issues with responsive design, accessibility, visual consistency, and layout breakage, then performs fixes at the source code level.
metadata:
...
## Cafe Debug Source of Truth (Mandatory)

When used in this repository, the visual source of truth is `cafedebug.pen` at the repository root.

Before reviewing or fixing any web UI:

1. Read `.specs/web/foundation/ux-design-reference.md` to identify the target screen section and Pencil node ID.
2. Use Pencil MCP to inspect the source design (`get_editor_state`, `batch_get`, and `get_screenshot`).
3. Compare implementation against the matching Pencil node in both light and dark themes and relevant breakpoints.

Do not approve or "polish" UI based on subjective judgment when a Pencil reference exists. Align to `cafedebug.pen`.
...
| Pencil Mapping | Which `cafedebug.pen` node ID maps to this UI? |

### 1.3 Pencil Baseline Collection (Cafe Debug)

For this repository, this step is required:

1. Identify node IDs using `.specs/web/foundation/ux-design-reference.md`.
2. Capture reference screenshots from `cafedebug.pen` for the mapped nodes.
3. Record the node IDs in the review notes before changing code.
...
### 1.5 Identifying Styling Method
...
First compare against `cafedebug.pen` (layout, spacing, typography scale, component anatomy, token usage, and theme parity), then run the generic checks below.
...
5. **Match Pencil First**: If `cafedebug.pen` defines the UI, match it instead of inventing alternatives
...
4. Compare updated UI against the mapped `cafedebug.pen` node screenshots
...
| Pencil Source | `cafedebug.pen` |
| Pencil Node IDs | {comma-separated node IDs reviewed} |
| Issues Detected | {N} |
| Issues Fixed | {M} |
